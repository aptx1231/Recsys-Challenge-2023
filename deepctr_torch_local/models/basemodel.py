...
class Linear(nn.Module):
    ...
    def forward(self, X, sparse_feat_refine_weight=None):
        ...
        if len(sparse_embedding_list) > 0:
            sparse_embedding_cat = torch.cat(sparse_embedding_list, dim=-1)  # （B, 1, fea)
...
class BaseModel(nn.Module):
    ...
    def fit(self, x=None, y=None, batch_size=None, epochs=1, verbose=1, initial_epoch=0, validation_split=0.,
        ...
        if self.gpus:
            self.logger.info('parallel running on these gpus: {}'.format(self.gpus))
        ...
        # Train
        self.logger.info("Train on {0} samples, validate on {1} samples, {2} steps per epoch".format(
        ...
        for epoch in range(initial_epoch, epochs):
            ...
            try:
                with tqdm(enumerate(train_loader), disable=verbose != 1, total=steps_per_epoch) as t:
                    for _, (x_train, y_train) in t:
                        x = x_train.to(self.device).float()  # (B, fea)
                        y = y_train.to(self.device).float()  # (B, 1)
            ...
            if verbose > 0:
                ...
                self.logger.info('Epoch {0}/{1}'.format(epoch + 1, epochs))
    ...
    def input_from_feature_columns(self, X, feature_columns, embedding_dict, support_dense=True):
        # X: (B, len_sparse_fea + len_dense_fea), len(feature_columns) = len_sparse_fea + len_dense_fea, len(embedding_dict) = len_sparse_fea
        ...
        sparse_embedding_list = [embedding_dict[feat.embedding_name](
            X[:, self.feature_index[feat.name][0]:self.feature_index[feat.name][1]].long()) for
            feat in sparse_feature_columns]  # list of (B, 1, sparse_dim)
        ...
        dense_value_list = [X[:, self.feature_index[feat.name][0]:self.feature_index[feat.name][1]] for feat in
                            dense_feature_columns]  # list of (B, 1, dimension)
