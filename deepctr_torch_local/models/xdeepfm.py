...
class xDeepFM(BaseModel):
    ...
    def forward(self, X):
        ...
        linear_logit = self.linear_model(X)  # (B, 1)
        if self.use_cin:
            cin_input = torch.cat(sparse_embedding_list, dim=1)  # (B, len_sp_fea, sparse_dim)
            cin_output = self.cin(cin_input)
            cin_logit = self.cin_linear(cin_output)  # (B, 1)
        if self.use_dnn:
            dnn_input = combined_dnn_input(sparse_embedding_list, dense_value_list)  # (B, len_sp_fea * sparse_dim)
