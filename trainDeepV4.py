...
"""
稀疏特征加入不同的维度sparse_dim，根据类别数确定
稠密特征分桶加入不同的桶数dense_bin，根据特征的最大值确定
"""
...
data_path = 'data/data_dense_bins{}_{}_adaptive_2_4_8.csv'.format(dense_bins, strategy)
...
if os.path.exists(data_path):
    ...
    dense_bins_list = []  # 不能nunique，因为可能这种分桶，某一桶是空的，得是max
    for feat in dense_features:
        if data[feat].max() - data[feat].min() <= 10:
            dense_bins_list.append(dense_bins)
        elif data[feat].nunique() <= 100:
            dense_bins_list.append(dense_bins * 2)
        elif data[feat].nunique() <= 1000:
            dense_bins_list.append(dense_bins * 4)
        else:
            dense_bins_list.append(dense_bins * 8)
    logger.info('dense_bins_list: {}'.format(dense_bins_list))
else:
    ...
    add_sparse_features = []
    for i, fea in tqdm(enumerate(dense_features), total=len(dense_features)):
        discretizer = KBinsDiscretizer(n_bins=dense_bins_list[i], encode='ordinal', strategy=strategy, random_state=seed)  # 等频quantile，等宽uniform
...
for feat in sparse_features:
    if data[feat].nunique() <= 10:
        sparse_dim_list.append(sparse_dim)
    elif data[feat].nunique() <= 100:
        sparse_dim_list.append(sparse_dim * 4)
    elif data[feat].nunique() <= 1000:
        sparse_dim_list.append(sparse_dim * 8)
    else:
        sparse_dim_list.append(sparse_dim * 16)

add_sparse_dim_list = []
for feat in add_sparse_features:
    if data[feat].max() <= 10:
        add_sparse_dim_list.append(sparse_dim)
    elif data[feat].max() <= 100:
        add_sparse_dim_list.append(sparse_dim * 4)
    elif data[feat].max() <= 1000:
        add_sparse_dim_list.append(sparse_dim * 8)
    else:
        add_sparse_dim_list.append(sparse_dim * 16)

logger.info('sparse_dim_list: {}'.format(sparse_dim_list))
logger.info('add_sparse_dim_list: {}'.format(add_sparse_dim_list))

if add_dense:
    mms = MinMaxScaler(feature_range=(0,1))
    data[dense_features] = mms.fit_transform(data[dense_features])
    fixlen_feature_columns = \
        [SparseFeat(feat, vocabulary_size=data[feat].nunique(), embedding_dim=sparse_dim_list[i]) for i, feat in enumerate(sparse_features)] + \
        [SparseFeat(feat, vocabulary_size=dense_bins_list[i], embedding_dim=add_sparse_dim_list[i]) for i, feat in enumerate(add_sparse_features)] + \
